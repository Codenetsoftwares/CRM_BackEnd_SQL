...
const EditAPIRoute = (app) => {
  ...
  app.put('/api/bank-edit/:bank_id', Authorize(['superAdmin', 'Bank-View', 'Transaction-View']), async (req, res) => {
    try {
      ...
      const [id] = await database.execute(`SELECT * FROM Bank WHERE (bank_id) = (?)`, [bankId]);
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const [editRequest] = await database.execute(`SELECT * FROM EditBankRequest WHERE bank_id = ?`, [
        ...
        if (!editRequest[0].isApproved) {
          if (isApproved) {
            const [bankExists] = await database.execute(`SELECT * FROM Bank WHERE bankName = ? AND bank_id != ?`, [
            ...
            await database.execute(`UPDATE EditBankRequest SET isApproved = TRUE WHERE bank_id = ?`, [
              req.params.requestId,
            ]);

            await database.execute(`DELETE FROM EditBankRequest WHERE bank_id = ?`, [req.params.requestId]);
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        const [editWebsite] = await database.execute(`SELECT * FROM Website WHERE (website_id) = (?)`, [id]);
        if (!editWebsite) {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const [editRequest] = await database.execute(`SELECT * FROM EditWebsiteRequest WHERE website_id = ?`, [
          req.params.requestId,
        ]);
        ...
        if (!editRequest[0].isApproved) {
          if (isApproved) {
            ...
            await database.execute(`UPDATE Website SET websiteName = ? WHERE website_id = ?`, [
              editRequest[0].websiteName,
              req.params.requestId,
            ]);

            await database.execute(`UPDATE EditWebsiteRequest SET isApproved = TRUE WHERE website_id = ?`, [
              req.params.requestId,
            ]);

            await database.execute(`DELETE FROM EditWebsiteRequest WHERE website_id = ?`, [req.params.requestId]);
